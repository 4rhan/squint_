"""Collect scripted/motion-planning demonstrations for SO101Stack3Cube-v1.

Long-horizon 3-cube stack (envs/stack3.py): itemB onto itemC, then itemA
onto itemB, using the scripted grasp solver in
examples/motionplanning/so101/motionplanner.py.
...
`python -m mani_skill.trajectory.replay_trajectory`, e.g.:
    ...
    python -m mani_skill.trajectory.replay_trajectory \\
        --traj-path demos/SO101Stack3Cube-v1/motionplanning/trajectory.h5 \\
        --save-traj --obs-mode rgb+state \\
        --target-control-mode pd_joint_target_delta_pos \\
        --num-procs 4

Usage:
    python -m examples.collect_stack3_demos -n 200 --only-count-success
    python -m examples.collect_stack3_demos -n 1 --vis --save-video  # sanity check
"""
import argparse
import os.path as osp
import time

import envs  # noqa: F401  registers SO101Stack3Cube-v1 etc.
import gymnasium as gym
import numpy as np
from tqdm import tqdm

from examples.motionplanning.so101.solutions.stack3_cube import solve
from mani_skill.utils.wrappers.record import RecordEpisode


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("-e", "--env-id", type=str, default="SO101Stack3Cube-v1")
...
def main(args):
    ...
    env = RecordEpisode(
        ...
        source_desc="scripted SO101 stack3_cube solution (IK + joint interpolation)",
        video_fps=30,
        record_reward=False,
        save_on_reset=False,
    )
