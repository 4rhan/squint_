...
@register_env("SO101ToolSweep-v1", max_episode_steps=100)
class ToolSweep(DefaultCameraEnv):
    """
    **Task Description:**
    ...
    **Success Conditions:**
    ...
    def _load_scene(self, options: dict):
        ...
        # 1. Build the Sweeper Tool (T-Shape), scaled for the SO100/SO101 desktop arms
        sweepers = []
        for i in range(self.num_envs):
            ...
            builder.add_cylinder_collision(radius=0.008, half_length=0.045, pose=handle_pose, material=mat, density=500)
            builder.add_cylinder_visual(radius=0.008, half_length=0.045, pose=handle_pose,
                                        material=sapien.render.RenderMaterial(base_color=[0.8, 0.6, 0.2, 1]))

            # Blade (attached to the bottom of the handle)
            blade_pose = sapien.Pose([0, 0, -0.045])
            builder.add_box_collision(pose=blade_pose, half_size=[0.006, 0.035, 0.012], material=mat, density=1000)
            builder.add_box_visual(pose=blade_pose, half_size=[0.006, 0.035, 0.012],
                                   material=sapien.render.RenderMaterial(base_color=[0.2, 0.2, 0.2, 1]))

            builder.initial_pose = sapien.Pose(p=[0.2, 0.2, 0.1])  # Offset to avoid collision with puck at creation
            sweeper = builder.build(name=f"sweeper-{i}")
            self.remove_from_state_dict_registry(sweeper)
            sweepers.append(sweeper)
        self.sweeper = Actor.merge(sweepers, name="sweeper")
        self.add_to_state_dict_registry(self.sweeper)
        self.sweeper_handle_half_length = 0.045
        self.sweeper_blade_offset = 0.045
        ...
        for i in range(self.num_envs):
            ...
            builder.add_cylinder_collision(radius=0.02, half_length=0.01, pose=puck_pose, material=mat, density=200)
            builder.add_cylinder_visual(radius=0.02, half_length=0.01, pose=puck_pose,
                                        material=sapien.render.RenderMaterial(base_color=[0.1, 0.5, 0.8, 1]))

            builder.initial_pose = sapien.Pose(p=[-0.2, 0.2, 0.1])  # Offset to avoid collision with sweeper at creation
            puck = builder.build(name=f"puck-{i}")
            self.remove_from_state_dict_registry(puck)
            pucks.append(puck)
        self.puck = Actor.merge(pucks, name="puck")
        self.add_to_state_dict_registry(self.puck)
        self.puck_radius = 0.02
        ...
        goal_builder.add_cylinder_visual(radius=0.05, half_length=0.001, pose=goal_pose,
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        with torch.device(self.device):
            ...
            sweeper_xy = sampler.sample(0.06, 100)
            sweeper_xyz = torch.zeros((b, 3))
            sweeper_xyz[:, :2] = spawn_center[env_idx, :2] + sweeper_xy
            sweeper_xyz[:, 2] = 0.008  # Radius of handle
            ...
            # Place Puck
            puck_xy = sampler.sample(0.03, 100, verbose=False)
            puck_xyz = torch.zeros((b, 3))
            puck_xyz[:, :2] = spawn_center[env_idx, :2] + puck_xy
            puck_xyz[:, 2] = 0.01
            self.puck.set_pose(Pose.create_from_pq(puck_xyz))

            # Place Goal Site
            goal_xy = sampler.sample(0.06, 100, verbose=False)
            goal_xyz = torch.zeros((b, 3))
            goal_xyz[:, :2] = spawn_center[env_idx, :2] + goal_xy
            goal_xyz[:, 2] = 0.001
    ...
    def evaluate(self):
        ...
        # Check if puck is inside the goal radius
        puck_to_goal = torch.linalg.norm(puck_pos[:, :2] - goal_pos[:, :2], axis=1)
        is_puck_in_goal = puck_to_goal < self.goal_radius
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: dict):
        ...
        blade_local = Pose.create_from_pq(p=torch.tensor([0.0, 0.0, -self.sweeper_blade_offset], device=self.device))
        ...
        valid_sweep = is_grasped & (blade_to_puck < 0.1)
