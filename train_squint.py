    ...
    class Args:
        ...
        chunk_size: int = 1
        """number of actions (h) the actor predicts per decision step. h=1 reproduces vanilla SAC exactly.
        global_step still counts raw env steps: one decision step advances global_step by num_envs*chunk_size."""
        smooth_coef: float = 0.1
        """weight (beta) on the intra-chunk action-smoothness penalty in the actor loss. No-op when chunk_size=1."""
    ...
    def evaluate(args, eval_envs, get_action_fn, logger, eval_output_dir, max_episode_steps, global_step, pbar):
        ...
        # Mirror the training rollout: query the policy once per h-step chunk, execute it open-loop.
        num_decision_steps = max_episode_steps // args.chunk_size
        for _ in range(num_decision_steps):
            with torch.no_grad():
                flat_chunk = get_action_fn(eval_obs['rgb'], eval_obs['state'])
                chunk = flat_chunk.view(flat_chunk.shape[0], args.chunk_size, -1)
                for i in range(args.chunk_size):
                    eval_obs, _, _, _, eval_infos = eval_envs.step(chunk[:, i, :])
    ...
    class Actor(nn.Module):
        def __init__(self, env, n_obs, n_state, n_act, chunk_size=1, device=None):
            super().__init__()
            hidden_dim = 256
            activ = nn.ReLU
            ...
            self.fc_mean = nn.Linear(hidden_dim, n_act_chunk, device=device)
            self.fc_logstd = nn.Linear(hidden_dim, n_act_chunk, device=device)

            action_space = env.unwrapped.single_action_space
            self.register_buffer("action_scale",
                torch.tensor((action_space.high - action_space.low) / 2.0, dtype=torch.float32, device=device).repeat(chunk_size))
    ...
    class DeployAgent(nn.Module):
        ...
        def __init__(self, sim_env, sample_obs, target_image_size=16, chunk_size=1, device=None):
            super().__init__()
            self.device = device
            ...
            self.actor = Actor(sim_env, n_obs=self.encoder.repr_dim, n_state=n_state, n_act=n_act, chunk_size=chunk_size, device=self.device)
        ...
        def load_checkpoint(self, checkpoint, checkpoint_config=None, version=None):
            ...
            ckpt_chunk_size = ckpt.get('chunk_size', 1)
            assert ckpt_chunk_size == self.chunk_size, (
                f"Checkpoint was trained with chunk_size={ckpt_chunk_size}, but DeployAgent was constructed with "
                f"chunk_size={self.chunk_size}. Pass --chunk_size={ckpt_chunk_size} to match."
            )
        ...
        def get_action_chunk(self, obs):
            """Returns the predicted action chunk reshaped to (chunk_size, n_act), batch dim squeezed
            (real-robot deployment always runs a single instance, i.e. batch size 1)."""
            flat_action = self.get_action(obs)
            return flat_action.view(-1, self.chunk_size, self.n_act).squeeze(0)
    ...
    if __name__ == "__main__":
        args = tyro.cli(Args)
        args.num_total_iterations = int(args.total_timesteps // (args.num_envs * args.chunk_size))
        ...
        actor = Actor(envs, n_obs=encoder.repr_dim, n_state=n_state, n_act=n_act, chunk_size=args.chunk_size, device=device)
        critic = Critic(n_obs=encoder.repr_dim, n_state=n_state, n_act=n_act * args.chunk_size,
                        num_atoms=args.num_atoms, v_min=args.v_min, v_max=args.v_max,
                        num_q=args.num_q, device=device)

        # Entropy tuning
        if args.autotune:
            target_entropy = -torch.prod(torch.Tensor(envs.unwrapped.single_action_space.shape).to(device)).item() * args.chunk_size
        ...
        actor_detach = Actor(envs, n_obs=encoder.repr_dim, n_state=n_state, n_act=n_act, chunk_size=args.chunk_size, device=device)
        actor_eval = Actor(envs, n_obs=encoder.repr_dim, n_state=n_state, n_act=n_act, chunk_size=args.chunk_size, device=device).eval()
        from_module(actor).data.to_module(actor_detach)
        from_module(actor).data.to_module(actor_eval)

        # Target critic 
        critic_target = Critic(n_obs=encoder.repr_dim, n_state=n_state, n_act=n_act * args.chunk_size,
                            num_atoms=args.num_atoms, v_min=args.v_min, v_max=args.v_max,
                            num_q=args.num_q, device=device)
        ...
        buffer_mem = utils.calc_buffer_memory(
            ...
            state_dim=n_state,
            action_dim=n_act * args.chunk_size,
            max_length=min(args.buffer_size, args.total_timesteps // args.chunk_size),
            rgb_dtype=np.uint8,
        )
        ...
        def update_main(data):
            with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                with torch.no_grad():
                    ...
                    discount = args.gamma ** args.chunk_size
        ...
        def update_actor(data, encoded_rgb):
            with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                ...
                # Intra-chunk smoothness penalty: sum of squared L2 distances between consecutive
                # actions in the predicted chunk. Zero when chunk_size==1 (chunk[:, 1:, :] is empty).
                chunk = pi.view(pi.shape[0], args.chunk_size, n_act)
                diffs = chunk[:, 1:, :] - chunk[:, :-1, :]
                l_smooth = diffs.pow(2).sum(dim=(-1, -2))

                actor_loss = (alpha * log_pi.squeeze(-1) - critic_value + args.smooth_coef * l_smooth).mean()
            ...
            return TensorDict(actor_loss=actor_loss.detach(), l_smooth=l_smooth.mean().detach())
        ...
        for iteration in range(args.num_total_iterations + 2):  # +2 for final eval
            # Evaluate
            if args.eval_freq > 0 and ((global_step - args.num_envs * args.chunk_size) // args.eval_freq) < (global_step // args.eval_freq):
            ...
            # Collect: query the policy once per decision step, execute the whole h-action chunk
            obs_t = obs
            if global_step < args.learning_starts:
                # h independent single-step random actions (diverse exploration within the chunk,
                # rather than repeating one random action h times)
                chunk_actions = torch.stack([
                    torch.as_tensor(envs.action_space.sample(), device=device, dtype=torch.float)
                    for _ in range(args.chunk_size)
                ], dim=1)  # [B, h, n_act]
            else:
                flat_chunk = get_rollout_action(obs_t['rgb'], obs_t['state'])  # [B, h*n_act]
                chunk_actions = flat_chunk.view(flat_chunk.shape[0], args.chunk_size, n_act)

            R_chunk = torch.zeros(chunk_actions.shape[0], device=device)
            chunk_final_infos = []  # episode-completion infos from any sub-step, for logging (§ Log block below)

            for i in range(args.chunk_size):
                actions_i = chunk_actions[:, i, :]
                next_obs, rewards_i, terminations, truncations, infos = envs.step(actions_i)
                R_chunk = R_chunk + (args.gamma ** i) * rewards_i
                if "final_info" in infos:
                    chunk_final_infos.append(infos)

            real_next_obs = {'rgb': next_obs['rgb'].clone(), 'state': next_obs['state'].clone()}

            # Determine bootstrap behavior, based on the LAST sub-step of the chunk only (simple variant:
            # intra-chunk terminations on earlier sub-steps are not specially tracked here, see notes)
            ...
            transition = TensorDict(
                ...
                actions=chunk_actions.reshape(chunk_actions.shape[0], -1),
                rewards=R_chunk,
                dones=dones,
                batch_size=R_chunk.shape[0],
                device=device,
            )
            ...
            # Log (any sub-step within the chunk may have completed an episode)
            for sub_infos in chunk_final_infos:
                final_info = sub_infos["final_info"]
                done_mask = sub_infos["_final_info"]
                for k, v in final_info["episode"].items():
                    d[f"train/{k}"] = v[done_mask].float().mean()
                # logging for terminal bar
                max_ep_ret = max(final_info["episode"]["return"][done_mask])
                avg_returns.extend(final_info["episode"]["return"][done_mask])

            if chunk_final_infos:
            ...
            # Increment counters
            pbar.update(args.num_envs * args.chunk_size)
            global_step += args.num_envs * args.chunk_size
